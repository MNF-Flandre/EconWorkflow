...
DEFAULT_ROLE_SPECS: dict[str, RoleSpec] = {
    ...
    "c1_designer": RoleSpec(
        ...
        default_task="将假设映射为具体变量、定义样本选择、固定效应、聚类，产出不可修改的规格冻结文档",
        context_files=("phase_1/hypotheses.yaml",),
    ),
    "c2_data_auditor": RoleSpec(
        name="Phase 2：数据可得性审计员",
        layer="econ-os",
        deliverable="phase_2/c2_data_availability_report.md",
        default_task="验证规格中每个变量的可得性、数据源、口径一致性，确保merge_coverage > 40%",
    ...
    "d1_engineer": RoleSpec(
        ...
        default_task="按照design_lock.yaml执行数据清洗、合并、滞后结构化、Winsorize，产出分析用数据集",
        context_files=("phase_2/design_lock.yaml",),
    ),
    "d2_qa_auditor": RoleSpec(
        name="Phase 3：数据质控审计员",
        layer="econ-os",
        deliverable="phase_3/data_audit_scorecard.json",
        default_task="运行数据质量审计：样本流失树、主键唯一性、缺失值分布。判定merge_coverage是否 >= 0.85，决定是否进入Phase 4或回滚",
    ...
    "e1_runner": RoleSpec(
        ...
        default_task="按照design_lock.yaml执行基准回归（OLS/FE/RE），应用指定的固定效应和聚类",
    ...
    "e2_adversarial_auditor": RoleSpec(
        ...
        default_task="启动4项压力测试（聚类、安慰剂、异常值、子样本），计算robustness_pass_rate，判定是否满足 >= 0.50 threshold进入Phase 5或回滚",
    ...
    "f1_narrator": RoleSpec(
        ...
        default_task="对比假设与结果，解释系数的经济含义，撰写最终报告草稿",
    ...
    "f2_journal_reviewer": RoleSpec(
        ...
        default_task="模拟Top期刊审稿人进行严厉批评：检查内生性、过度解释、P-hacking。产出学术质量评估报告",
